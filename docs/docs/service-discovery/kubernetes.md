...
!!! note
    ...
    If the target Service is headless, Stork returns an empty list of service instances.
...
Note that: 
 ...
 - In case of an error contacting the cluster (e.g., network failure, API server unreachable), a retry mechanism is triggered. 
...
By default, retries are disabled to prevent the system from entering an infinite loop of calls to an unresponsive cluster.
 - If no service instances are found (e.g., no matching endpoints, headless service in ClusterIP mode), Stork returns an empty list.
