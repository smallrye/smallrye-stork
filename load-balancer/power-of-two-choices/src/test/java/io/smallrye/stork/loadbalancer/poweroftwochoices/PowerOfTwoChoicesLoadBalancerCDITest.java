...
public class PowerOfTwoChoicesLoadBalancerCDITest {
    private static final Logger log = LoggerFactory.getLogger(PowerOfTwoChoicesLoadBalancerCDITest.class);
    ...
    public WeldInitiator weld = WeldInitiator.of(TestConfigProviderBean.class,
            PowerOfTwoChoicesLoadBalancerProviderLoader.class);
    ...
    void setUp() {
        config.clear();
        config.addServiceConfig("first-service", "power-of-two-choices", "static",
                null,
                Map.of("address-list", String.format("%s,%s,%s,%s", FST_SRVC_1, FST_SRVC_2, FST_SRVC_3, FST_SRVC_4)));
        config.addServiceConfig("first-service-secure-random", "power-of-two-choices", "static",
                Map.of("use-secure-random", "true"),
                Map.of("address-list", String.format("%s,%s,%s,%s", FST_SRVC_1, FST_SRVC_2, FST_SRVC_3, FST_SRVC_4)));
        config.addServiceConfig("singleton-service", "power-of-two-choices",
                "static", null,
                Map.of("address-list", FST_SRVC_1));
        config.addServiceConfig("without-instances", "power-of-two-choices",
